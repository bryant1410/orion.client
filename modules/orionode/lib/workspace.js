	...
	function getWorkspaceJson(req, workspace) {
		...
		.then(function(children) {
			...
			return Promise.all(fileUtil.getDecorators().map(function(decorator) {
				return decorator.decorate(req, {workspaceDir: workspaceDir, path: ""}, workspaceJson);
			}));
		}).then(function() {
		...
		if (rest === '') {
			...
			store.getWorkspace(workspaceId, function(err, workspace) {
				...
				var file = {
					...
					path: fileUtil.safeFilePath(workspaceDir, projectName)
				};
				if (req.body && req.body.Location) {//TODO wrond check -> options
				...
				// Create a project
				fs.mkdir(file.path, parseInt('0755', 8), function(error) {
					if (error) {
						if (error.code === 'EEXIST') {
							writeError(400, res, "Duplicate project name: " + projectName);
						} else {
