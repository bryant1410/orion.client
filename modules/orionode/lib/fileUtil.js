...
exports.getFile = function(req, rest) {
	if (!rest) return null;
...
exports.handleFilePOST = function(workspaceRoot, fileRoot, req, res, destFile, metadataMixins, statusCode) {
	...
	.then(function(destExists) {
		...
		if (isCopy || isMove) {
			...
			var sourceFilepath = safeFilePath(destFile.workspaceDir, api.rest(fileRoot, api.matchHost(req, sourceUrl)));
			return fs.statAsync(sourceFilepath)
			.then(function(/*stats*/) {
				return isCopy ? copy(sourceFilepath, destFile.path) : fs.renameAsync(sourceFilepath, destFile.path);
			})
